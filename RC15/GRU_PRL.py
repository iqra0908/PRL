...
def parse_args():
    parser = argparse.ArgumentParser(description="Run GRU with PRL.")
...
class GRU_PRLnetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, max_step):
        ...
        self.seq = tf.stack([self.states_hidden, self.step_embeddings, self.reward_embedding], axis=1)
        ...
        # self.seq=tf.contrib.layers.fully_connected(self.seq, self.hidden_size, activation_fn=None,
        #                                                 scope='mlp')
        # s=tf.reduce_mean(self.seq,axis=1)
...
def evaluate(sess,filename='sampled_val.df'):
    eval_sessions = pd.read_pickle(os.path.join(data_directory, filename))
    ...
    while evaluated < len(eval_ids):
        ...
        for i in range(batch):
            ...
            for index, row in group.iterrows():
                ...
                # reward_to_go.append(np.random.normal(r_t_g, args.std)*reward_mean[step])
            ...
            if evaluated == len(eval_ids):
                break
        prediction = sess.run(GRU_PRLnet.output, feed_dict={GRU_PRLnet.inputs: states, GRU_PRLnet.len_state: len_states,
                                                            GRU_PRLnet.reward_to_go: reward_to_go,
                                                            GRU_PRLnet.timestamp: timestamp,
                                                            # GRU_PRLnet.cls: cls,
                                                            GRU_PRLnet.is_training: False
                                                            })
    ...
    logging.info('#############################################################')
    return hit_purchase[1] / total_purchase
...
if __name__ == '__main__':
    ...
    save_file = 'GRU-PRL:{}'.format(Time.strftime("%m-%d %H:%M:%S", Time.localtime()))

    tf.reset_default_graph()

    logging.basicConfig(
        filename="./log/GRU/head:{}_block:{}_drop:{}_rtg:{}_std:{}_lr:{}_{}".format(
    ...
    GRU_PRLnet = GRU_PRLnetwork(hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                for k in range(args.batch_size):
                    ...
                    # weight.append(reward_to_go[k])
                # cls=[0]*len(timestamp)
                loss, _ = sess.run([GRU_PRLnet.loss, GRU_PRLnet.opt],
                                   feed_dict={GRU_PRLnet.inputs: state,
                                              GRU_PRLnet.len_state: len_state,
                                              GRU_PRLnet.target: target,
                                              GRU_PRLnet.reward_to_go: reward_to_go,
                                              GRU_PRLnet.timestamp: timestamp,
                                              GRU_PRLnet.weight:weight,
                                              # GRU_PRLnet.cls:cls,
                                              GRU_PRLnet.is_training: True})
                total_step += 1
                best_rec=0
                if total_step % 200 == 0:
                ...
                if total_step % 2000 == 0:
                    hr_purchase=evaluate(sess)
                    if hr_purchase>best_rec:
                        best_rec=hr_purchase
                        saver.save(sess, save_file)

        # saver.restore(sess,save_file)
        # evaluate(sess,'sampled_test.df')
