...
def parse_args():
    parser = argparse.ArgumentParser(description="Run NItNet with PRL.")
...
class NItNet_PRLnetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, max_step):
        ...
        context_embedding = tf.nn.embedding_lookup(all_embeddings['state_embeddings'],
                                                   self.inputs)
        ...
        self.seq = tf.stack([self.state_hidden, self.step_embeddings, self.reward_embedding], axis=1)
    ...
    while evaluated < len(eval_ids):
        ...
        for i in range(batch):
            ...
            if evaluated == len(eval_ids):
                break
        prediction = sess.run(NItNet_PRLnet.output, feed_dict={NItNet_PRLnet.inputs: states, NItNet_PRLnet.len_state: len_states,
                                                            NItNet_PRLnet.reward_to_go: reward_to_go,
                                                            NItNet_PRLnet.timestamp: timestamp,
                                                            # NItNet_PRLnet.cls: cls,
                                                            NItNet_PRLnet.is_training: False
                                                            })
...
if __name__ == '__main__':
    ...
    # save_file = 'pretrain-NItNet/%d' % (hidden_size)

    tf.reset_default_graph()

    logging.basicConfig(
        filename="./log/NItNet/head:{}_block:{}_drop:{}_rtg:{}_std:{}_lr:{}_{}".format(
    ...
    NItNet_PRLnet = NItNet_PRLnetwork(hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                # cls=[0]*len(timestamp)
                loss, _ = sess.run([NItNet_PRLnet.loss, NItNet_PRLnet.opt],
                                   feed_dict={NItNet_PRLnet.inputs: state,
                                              NItNet_PRLnet.len_state: len_state,
                                              NItNet_PRLnet.target: target,
                                              NItNet_PRLnet.reward_to_go: reward_to_go,
                                              NItNet_PRLnet.timestamp: timestamp,
                                              NItNet_PRLnet.weight:weight,
                                              # NItNet_PRLnet.cls:cls,
                                              NItNet_PRLnet.is_training: True})
                total_step += 1
                if total_step % 200 == 0:
